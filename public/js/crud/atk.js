...
//EDIT tamu
    $('body').on('click', '.edit', function () {
        var id = $(this).data('id');
        $.get("data-atk" +'/' + id +'/edit', function (data) {
            $('#modelHeading').html("Edit ATK");
            ...
            success: function (data) {
                console.log(data.error)
                    if($.isEmptyObject(data.error)){
                        ...
                        table.draw();
                        toastr.success('Berhasil Menyimpan ATK', 'Success !'),(data.success);
                    }else{
        ...
        var result = Swal.fire({
            ...
            showCancelButton: true,
                ...
                if (result.isConfirmed){
                    ...
                    success: function (data) {
                        table.draw();
                        toastr.success('Berhasil Menghapus ATK', 'Success !'),(data.success);
                        $('#formCreate').find('.help-block').remove();
                        $('#formCreate').find('.col-sm-9').removeClass('.has-error');
                    },
                    error: function (data) {
